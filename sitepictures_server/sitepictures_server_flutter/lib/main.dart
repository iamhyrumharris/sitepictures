...
class MyHomePageState extends State<MyHomePage> {
  ...
  /// Calls the `getAllCompanies` method of the `company` endpoint. Will set either the
  /// `_resultMessage` or `_errorMessage` field, depending on if the call
  /// is successful.
  void _callHello() async {
    try {
      final result = await client.company.getAllCompanies(includeSystem: false);
      setState(() {
        _errorMessage = null;
        _resultMessage = 'Found ${result.length} companies';
      });
    } catch (e) {
      setState(() {
        _errorMessage = '$e';
