...
void main() {
  group(
    ...
    skip: 'Database initialization needs sqflite_common_ffi setup',
    () {
      testWidgets('Screen transitions should complete in less than 500ms', (
        ...
        expect(
          ...
          lessThan(500),
          reason:
              'Home → Client transition took ${transitionTime}ms, exceeds 500ms limit',
        );

        print(
          '✓ Home → Client transition: ${transitionTime}ms (target: <500ms)',
        );
      });
      ...
      testWidgets('Deep navigation path should maintain performance', (
        ...
        final avgTime =
            transitionTimes.values.reduce((a, b) => a + b) /
      ...
      testWidgets('Bottom navigation tab switching should be instant', (
        ...
        await tester.pumpAndSettle();
        stopwatch.stop();
        tabSwitchTimes['Home → All Photos'] = stopwatch.elapsedMilliseconds;
        ...
        await tester.pumpAndSettle();
        stopwatch.stop();
        tabSwitchTimes['All Photos → Settings'] = stopwatch.elapsedMilliseconds;
      ...
      testWidgets('Camera screen launch should be fast (from equipment)', (
        ...
        expect(
          ...
          lessThan(500),
          reason:
              'Camera launch took ${cameraLaunchTime}ms, exceeds 500ms limit',
        );
      ...
      testWidgets(
        'Navigation performance should not degrade with data volume',
      ...
      testWidgets('60 FPS UI rendering during navigation', (
        ...
        if (slowFrames.isNotEmpty) {
          ...
          print(
            '  Slow frames: ${slowFrames.map((d) => "${d.inMilliseconds}ms").join(", ")}',
          );
        } else {
        ...
        expect(
          ...
          lessThan(0.1),
          reason:
              'Too many slow frames: ${slowFrames.length}/${frameDurations.length}',
        );
      });
    },
  );
